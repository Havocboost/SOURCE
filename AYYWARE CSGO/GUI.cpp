...
{
	// Main Menu Colour
	int rmma = Menu::Window.GUITab.RMainMenuA.GetValue();
	int gmma = Menu::Window.GUITab.GMainMenuA.GetValue();
	int bmma = Menu::Window.GUITab.BMainMenuA.GetValue();

	Render::Outline(window->m_x, window->m_y, window->m_iWidth, window->m_iHeight, Color(21, 21, 21, 80));
	Render::GradientV(window->m_x + 2, window->m_y + 2, window->m_iWidth - 4, 26, Color(rmma, gmma, bmma, 255), Color(rmma, gmma, bmma, 255));
	Render::Clear(window->m_x + 2, window->m_y + 2 + 26, window->m_iWidth - 4, window->m_iHeight - 4 - 26, Color(rmma, gmma, bmma, 255));
	Render::Outline(window->m_x + 1, window->m_y + 1, window->m_iWidth - 2, window->m_iHeight - 2, Color(rmma, gmma, bmma, 255));
	...
	int rtext = Menu::Window.GUITab.RMenuText.GetValue();
	int gtext = Menu::Window.GUITab.GMenuText.GetValue();
	int btext = Menu::Window.GUITab.BMenuText.GetValue();

	Render::Text(window->m_x + 8, window->m_y + 8, Color(rtext, gtext, btext, 255), Render::Fonts::MenuBold, window->Title.c_str());

	// Inner
	// Render::Outline(window->m_x + 7, window->m_y + 1 + 26, window->m_iWidth - 4 - 10, window->m_iHeight - 2 - 6 - 26, Color(255, 255, 255, 80));

	int rinn = Menu::Window.GUITab.RInner.GetValue();
	int ginn = Menu::Window.GUITab.GInner.GetValue();
	int binn = Menu::Window.GUITab.BInner.GetValue();
	int ainn = Menu::Window.GUITab.AInner.GetValue();

	Render::Clear(window->m_x + 8, window->m_y + 1 + 27, window->m_iWidth - 4 - 12, window->m_iHeight - 2 - 8 - 26, Color(rinn, ginn, binn, ainn));

	// Tab

	int rtab = Menu::Window.GUITab.RTabs.GetValue();
	int gtab = Menu::Window.GUITab.GTabs.GetValue();
	int btab = Menu::Window.GUITab.BTabs.GetValue();

	Render::GradientV(window->m_x + 8, window->m_y + 1 + 27, window->m_iWidth - 4 - 12, 29, Color(rtab, gtab, btab, 255), Color(49, 42, 42, 255));
	int TabCount = window->Tabs.size();
	if (TabCount) // If there are some tabs
	{
		int TabSize = (window->m_iWidth - 4 - 12) / TabCount;
		for (int i = 0; i < TabCount; i++)
		{
			...
			{
				Render::GradientV(window->m_x + 8 + (i*TabSize), window->m_y + 1 + 27, TabSize, 29, Color(106, 106, 106, 255), Color(rtab, gtab, btab, 255));
			}
			...
			Render::Text(TabArea.left + (TabSize / 2) - (TextSize.right / 2), TabArea.top + 8, Color(rtext, gtext, btext, 255), Render::Fonts::MenuBold, tab->Title.c_str());
